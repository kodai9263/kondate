...
export function RecipeCookingGuide({ ingredients, morning, steps, baseServings, totalMinutes, notes = [], scalable = false }: RecipeCookingGuideProps) {
  ...
  const scaledIngredients = ingredients.map((line) => scaleRecipeIngredient(line, servings, baseServings));
  const seasoningGroups = getSeasoningGroups(scaledIngredients);
  return <div className="mt-6 space-y-7">
    ...
    <section aria-labelledby={ingredientsTitleId}>
      ...
      {seasoningGroups.length ? <p className="mt-3 text-sm leading-6 text-kondate-muted">A・Bなど同じ記号の材料をまとめて用意します。混ぜる順番や加えるタイミングは、作り方に沿って進めてください。</p> : null}
      {ingredients.length ? <ul className="mt-3 divide-y divide-kondate-line">{scaledIngredients.map((line, index) => <li key={index} className="py-3 text-[15px] leading-7"><RecipeIngredientLine text={line} /></li>)}</ul> : <p className="mt-3 text-sm text-kondate-muted">材料の登録がありません。元レシピの材料と分量を確認してください。</p>}
    ...
    {morning.length ? <StepList title="先にしておく仕込み" steps={morning} groups={seasoningGroups} /> : null}
    <StepList title={morning.length ? "仕込みの後の作り方" : "作り方"} steps={steps} groups={seasoningGroups} />
  </div>;
}
