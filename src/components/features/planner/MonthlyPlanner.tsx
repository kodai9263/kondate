  ...
  return (
    <main className="mx-auto min-h-dvh w-full max-w-6xl px-4 pb-28 pt-5 sm:px-6">
      ...
      <header className="flex flex-col gap-4 border-b border-kondate-line pb-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <label className="w-fit text-xs text-kondate-muted">調理時間の上限<select value={maxCookMinutes} disabled={isSaving} onChange={(event) => void changeMaxCookMinutes(Number(event.target.value))} className="mt-1 block min-h-11 min-w-32 rounded-lg border border-kondate-line bg-white px-3 text-sm text-kondate-ink outline-none transition-colors focus:border-kondate-accent disabled:opacity-40"><option value={20}>20分</option><option value={30}>30分</option><option value={45}>45分</option></select></label>
      </header>
