...
import { createSideDish } from "@/app/app/planner/actions";
...
export function SideDishPicker({ day, sideDishes, demo, onClose, onSelect, onCreated }: {
  day: PlannedDinner; sideDishes: SideDish[]; demo: boolean; onClose: () => void;
  ...
  return <PlannerDialog labelId="side-picker-title" onClose={() => { if (!busyRef.current) onClose(); }}>
    ...
    <div className="overflow-y-auto p-4">
      ...
      {showForm ? <form onSubmit={(event) => { event.preventDefault(); void create(); }} className="space-y-4">
        ...
        {sideDishes.map((dish) => <Button key={dish.id} variant="secondary" disabled={busy} onClick={() => void select("custom", dish)} className="min-h-16 justify-start text-left">{dish.name}</Button>)}
      </div><Button variant="secondary" disabled={busy} fullWidth className="mt-4" onClick={() => setShowForm(true)}>新しい副菜を作る</Button></>}
    </div>
  </PlannerDialog>;
}
