import { ArrowLeft, CreditCard, Link2, LogOut, Save, Send } from "lucide-react";
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ error?: string; success?: string; feedback?: string; invite?: string; save?: string }> }) {
  ...
  return (
    <main id="account-top" className="mx-auto min-h-dvh w-full max-w-[560px] scroll-mt-4 px-4 pb-16 pt-5">
      ...
      <Link href="/app" className="mb-5 inline-flex min-h-11 items-center gap-2 text-sm text-kondate-muted transition-colors hover:text-kondate-ink"><ArrowLeft size={18} aria-hidden="true" />今日画面へ</Link>
      <div className="mb-6 border-b border-kondate-line pb-5"><h1 className="font-mincho text-[26px] font-bold">家族と契約の設定</h1></div>
      ...
      <section className="rounded border border-kondate-line bg-white p-5">
        ...
        <form action={updateAccount} className="mt-4 space-y-4">
          <label className="block text-sm font-semibold">表示名 <span className="text-xs text-kondate-muted">（必須）</span><input name="displayName" required defaultValue={profile.display_name} autoComplete="name" className="mt-2 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink" /></label>
          <label className="block text-sm font-semibold">家族グループ名 <span className="text-xs text-kondate-muted">（必須）</span><input name="householdName" required defaultValue={household?.name ?? "わが家"} className="mt-2 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink" /></label>
          ...
          <fieldset className="border-t border-kondate-line pt-5"><legend className="flex items-center gap-2 px-1 font-semibold">家族の人数</legend><div className="mt-4 grid grid-cols-2 gap-3"><label className="text-sm font-semibold">大人<input name="adultCount" type="number" inputMode="numeric" min="1" max="10" required defaultValue={familySize.adultCount} className="mt-2 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink" /></label><label className="text-sm font-semibold">子ども<input name="childCount" type="number" inputMode="numeric" min="0" max="10" required defaultValue={familySize.childCount} className="mt-2 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink" /></label></div><p className="mt-3 text-xs leading-6 text-kondate-muted">{formatServingLabel(familySize)}。子どもは大人の0.6人前として献立と買い物を調整します。</p></fieldset>
          <label className="block border-t border-kondate-line pt-5 text-sm font-semibold"><span className="flex items-center gap-2">まとめ買いの曜日</span><select name="shoppingDay" defaultValue={shoppingDay} className="mt-3 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink">{shoppingWeekdays.map((weekday, index) => <option key={weekday} value={index}>{weekday}曜日</option>)}</select></label>
          <fieldset className="border-t border-kondate-line pt-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {commonAllergens.map((allergen) => <label key={allergen} className="flex min-h-11 items-center gap-2 rounded border border-kondate-line bg-white px-3 text-sm"><input type="checkbox" name="allergies" value={allergen} defaultChecked={selectedAllergies.has(allergen)} className="size-4 accent-kondate-accent" />{allergen}</label>)}
            </div>
            <label className="mt-4 block text-sm font-semibold">その他の食材<textarea name="customAllergies" defaultValue={customAllergies.join("\n")} maxLength={500} rows={3} placeholder="例：キウイ、山芋、青魚" className="mt-2 w-full rounded border border-kondate-line bg-white px-3 py-3 text-base font-normal leading-7 outline-none focus:border-kondate-ink" /></label>
            <p className="mt-3 text-xs leading-6 text-kondate-muted">献立候補との照合に使います。自動判定は補助機能のため、原材料表示は必ず確認してください。</p>
          </fieldset>
          <button type="submit" className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded bg-kondate-accent px-4 font-semibold text-white transition-colors hover:bg-kondate-accentDark"><Save size={18} />変更を保存</button>
        </form>
      </section>
      ...
      <section className="mt-4 rounded border border-kondate-line bg-white p-5">
        ...
        {paid ? <form action={createFamilyInvite} className="mt-4"><button type="submit" className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded bg-kondate-ink px-4 font-semibold text-white transition-colors hover:bg-kondate-accent"><Link2 size={18} />招待リンクを作成</button></form> : <Link href="/pricing?required=family_sharing" className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded bg-kondate-ink px-4 font-semibold text-white transition-colors hover:bg-kondate-accent"><CreditCard size={18} />家族プランで招待する</Link>}
      </section>

      <section className="mt-4 rounded border border-kondate-line bg-white p-5">
        <div><p className="flex items-center gap-2 font-semibold">アプリへのご意見</p><p className="mt-1 text-sm leading-6 text-kondate-muted">不具合や改善してほしいことを運営へ送れます。</p></div>
        {params.feedback === "thanks" ? <p role="status" className="mt-4 rounded bg-kondate-doneSoft p-3 text-sm">ご意見を受け付けました。ありがとうございます。</p> : null}
        {params.feedback === "error" ? <p role="alert" className="mt-4 rounded bg-kondate-alertSoft p-3 text-sm text-kondate-alert">送信できませんでした。内容を確認してもう一度お試しください。</p> : null}
        <form action={submitAppFeedback} className="mt-4 space-y-4"><label className="block text-sm font-semibold">種類<select name="category" defaultValue="improvement" className="mt-2 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink"><option value="bug">不具合</option><option value="improvement">改善してほしい</option><option value="praise">良かった</option></select></label><label className="block text-sm font-semibold">内容 <span className="text-xs text-kondate-muted">（必須）</span><textarea name="message" required maxLength={2000} rows={5} className="mt-2 w-full rounded border border-kondate-line bg-white p-3 text-base font-normal outline-none focus:border-kondate-ink" placeholder="気づいたことをご記入ください" /></label><button type="submit" className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded bg-kondate-ink px-4 font-semibold text-white transition-colors hover:bg-kondate-accent"><Send size={18} />フィードバックを送る</button></form>
      </section>

      <section className="mt-8 border-t border-kondate-line pt-5">
        <form action={signOut}><button type="submit" className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded border border-kondate-line bg-white font-semibold text-kondate-muted"><LogOut size={18} />ログアウト</button></form>
      </section>
    </main>
  );
}
