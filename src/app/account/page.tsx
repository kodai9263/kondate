  ...
  return (
    <main id="account-top" className="mx-auto min-h-dvh w-full max-w-[560px] scroll-mt-4 px-4 pb-16 pt-5">
        ...
        <form action={updateAccount} className="mt-4 space-y-4">
          ...
          <fieldset className="border-t border-kondate-line pt-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {commonAllergens.map((allergen) => <label key={allergen} className="flex min-h-11 items-center gap-2 rounded-lg border border-kondate-line bg-white px-3 text-sm"><input type="checkbox" name="allergies" value={allergen} defaultChecked={selectedAllergies.has(allergen)} className="size-4 accent-kondate-accent" />{allergen}</label>)}
            </div>
            <label className="mt-4 block text-sm font-semibold">その他の食材<textarea name="customAllergies" defaultValue={customAllergies.join("\n")} maxLength={500} rows={3} placeholder="例：キウイ、山芋、青魚" className="mt-2 w-full rounded-lg border border-kondate-line bg-white px-3.5 py-3 text-base font-normal leading-7 outline-none transition-colors focus:border-kondate-accent focus:ring-2 focus:ring-kondate-accent/15" /></label>
      ...
      <section className="mt-4 rounded border border-kondate-line bg-white p-5">
        ...
        <form action={submitAppFeedback} className="mt-4 space-y-4"><label className="block text-sm font-semibold">種類<select name="category" defaultValue="improvement" className="mt-2 min-h-12 w-full rounded-lg border border-kondate-line bg-white px-3.5 text-base font-normal outline-none transition-colors focus:border-kondate-accent focus:ring-2 focus:ring-kondate-accent/15"><option value="bug">不具合</option><option value="improvement">改善してほしい</option><option value="praise">良かった</option></select></label><label className="block text-sm font-semibold">内容 <span className="text-xs text-kondate-muted">（必須）</span><textarea name="message" required maxLength={2000} rows={5} className="mt-2 w-full rounded-lg border border-kondate-line bg-white p-3.5 text-base font-normal outline-none transition-colors focus:border-kondate-accent focus:ring-2 focus:ring-kondate-accent/15" placeholder="気づいたことをご記入ください" /></label><Button type="submit" variant="ink" fullWidth><Send size={18} aria-hidden="true" />フィードバックを送る</Button></form>
      </section>
