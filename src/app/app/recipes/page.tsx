...
export default async function RecipesPage({ searchParams }: { searchParams: Promise<{ created?: string; deleted?: string; error?: string }> }) {
  ...
  return <main className="mx-auto min-h-dvh w-full max-w-5xl px-4 pb-28 pt-5 sm:px-6"><header className="flex items-end justify-between gap-4 border-b border-kondate-line pb-5"><div><h1 className="font-mincho text-[26px] font-bold">メニュー</h1><p className="mt-1.5 text-sm text-kondate-muted">公式レシピと、わが家の料理。</p></div><Link href={paid ? "/app/recipes/new" : "/pricing?required=custom_recipes"} className="inline-flex min-h-12 shrink-0 items-center gap-2 rounded bg-kondate-accent px-4 text-sm font-semibold text-white transition-colors hover:bg-kondate-accentDark">{paid ? <Plus size={18} aria-hidden="true" /> : <Crown size={18} aria-hidden="true" />}{paid ? "登録" : "家族プラン"}</Link></header>
    ...
    {customRecipes && customRecipes.length > 0 ? <section className="mt-8"><h2 className="text-sm font-semibold">わが家のメニュー</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{customRecipes.map((recipe) => <RecipeCard key={recipe.id} id={recipe.id} name={recipe.name} minutes={recipe.cook_minutes} custom />)}</div></section> : <section className="mt-8 border-y border-kondate-line py-10 text-center"><p className="font-mincho text-lg font-bold">まだ自分のメニューはありません</p><p className="mt-2 text-sm text-kondate-muted">よく作る料理を登録すると、自動献立に混ぜられます。</p></section>}
    <section className="mt-10"><div className="flex items-baseline justify-between gap-3"><h2 className="text-sm font-semibold">公式バランスメニュー</h2><p className="text-xs tabular-nums text-kondate-faint">{officialNutritionRecipes.length}品</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{officialNutritionRecipes.map((recipe) => <RecipeCard key={recipe.id} name={recipe.name} minutes={recipe.cookMinutes} />)}</div></section>
  </main>;
}

function RecipeCard({ id, name, minutes, custom = false }: { id?: string; name: string; minutes: number; custom?: boolean }) { return <article className="rounded border border-kondate-line bg-white px-4 py-3"><div className="flex items-start justify-between gap-3"><h3 className="font-mincho text-base font-bold leading-snug">{name}</h3>{custom ? <span className="shrink-0 rounded-sm bg-kondate-accentSoft px-2 py-0.5 text-xs text-kondate-accent">わが家</span> : null}</div><div className="mt-1 flex items-center justify-between gap-2"><p className="text-xs tabular-nums text-kondate-faint">調理 {minutes}分</p>{custom && id ? <ArchiveRecipeButton recipeId={id} recipeName={name} /> : null}</div></article>; }
