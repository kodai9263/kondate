...
export default async function NewRecipePage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <main className="mx-auto min-h-dvh w-full max-w-3xl px-4 pb-28 pt-5 sm:px-6">
      ...
      <header className="mt-4 border-b border-kondate-line pb-5"><h1 className="font-mincho text-[26px] font-bold">自分のメニューを登録</h1><p className="mt-1.5 text-sm text-kondate-muted">登録した料理は、翌月の栄養バランス献立にも使われます。</p></header>
      ...
      <form action={createRecipe} className="mt-6 space-y-8">
        <fieldset className="space-y-4"><legend className="mb-4 text-sm font-semibold">料理の基本</legend>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><NumberField label="調理時間" name="cookMinutes" unit="分" defaultValue="25" /><label className="block text-sm font-semibold">主なたんぱく源 <Required /><select name="proteinSource" required defaultValue="meat" className="mt-2 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink"><option value="fish">魚</option><option value="meat">肉</option><option value="soy">大豆・豆腐</option><option value="egg">卵</option><option value="noodle">麺・その他</option></select></label></div>
...
function Field({ label, name, required = false, helper }: { label: string; name: string; required?: boolean; helper?: string }) { return <label className="block text-sm font-semibold">{label} {required ? <Required /> : null}<input name={name} required={required} className="mt-2 min-h-12 w-full rounded border border-kondate-line bg-white px-3 text-base font-normal outline-none focus:border-kondate-ink" />{helper ? <span className="mt-1 block text-xs font-normal leading-6 text-kondate-faint">{helper}</span> : null}</label>; }
function TextArea({ label, name, helper }: { label: string; name: string; helper?: string }) { return <label className="block text-sm font-semibold">{label} <Required /><textarea name={name} required rows={5} className="mt-2 w-full rounded border border-kondate-line bg-white p-3 text-base font-normal outline-none focus:border-kondate-ink" />{helper ? <span className="mt-1 block text-xs font-normal leading-6 text-kondate-faint">{helper}</span> : null}</label>; }
function NumberField({ label, name, unit, defaultValue, step = "1" }: { label: string; name: string; unit: string; defaultValue: string; step?: string }) { return <label className="block text-sm font-semibold">{label} <Required /><span className="mt-2 flex min-h-12 items-center rounded border border-kondate-line bg-white focus-within:border-kondate-ink"><input name={name} type="number" inputMode="decimal" min="0" step={step} required defaultValue={defaultValue} className="min-w-0 flex-1 bg-transparent px-3 text-base font-normal outline-none" /><span className="pr-3 text-xs font-normal text-kondate-faint">{unit}</span></span></label>; }
